...
class CAM(nn.Module):
    ...
    def forward(self, f1_norm, f2_norm):
        ...
        for i in range(f1_norm.shape[0]):
            ...
            aud_vis_fts = torch.cat((aud_fts, vis_fts), 1)  # joint representation of A-V features -> J
            a_t = self.affine_a(aud_vis_fts.transpose(0,1)) 
            att_aud = torch.mm(aud_fts.transpose(0,1), a_t.transpose(0,1))  # att_aud = X_a^T W_ja J = aud_fts^T W_ja J
            audio_att = self.tanh(torch.div(att_aud, math.sqrt(aud_vis_fts.shape[1])))  # joint correlation matrix (X_a, J) -> C_a (equation 1)
            ...
            vis_att = self.tanh(torch.div(att_vis, math.sqrt(aud_vis_fts.shape[1])))  # C_v (equation 2)

            H_a = self.relu(self.W_ca(audio_att) + self.W_a(aud_fts.transpose(0,1)))  # attention matrix/map -> H_a (equation 3)
            H_v = self.relu(self.W_cv(vis_att) + self.W_v(vis_fts.transpose(0,1)))  # attention matrix/map -> H_v (equation 4)

            att_audio_features = self.W_ha(H_a).transpose(0,1) + aud_fts  # X_att,a (equation 5)
            att_visual_features = self.W_hv(H_v).transpose(0,1) + vis_fts  # X_att,v (equation 6)
            ...
            audiovisualfeatures = torch.cat((att_audio_features, att_visual_features), 1)  # X_att (equation_7)
